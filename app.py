...
app.layout = html.Div(children=[
    ...
    html.Div(className="row app-body", children=[
        html.Div(className="twelve columns card", children=[
            html.Div(className="padding row", children=[
                html.Div(className="two columns card", children=[
                    html.H6("Select Exchange"),
                    dcc.RadioItems(id="exchange-select",
                    options=[
                ...
                html.Div(className="two columns card 2", children=[
                    html.H6("Select Leverage"),
                    dcc.RadioItems(id="leverage-select",
                    options=[
                    {'label': leverage, 'value': leverage} for leverage in df['Margin'].unique()
                    ],
                    value=1,
                    labelStyle={'display': 'inline-block'}
                    )
                ]),
                html.Div(className="three columns card", children=[
                ...
                html.Div(id="market-returns-div",
                className="two columns indicator pretty_container",
                children=[
        ...
        html.Div(className="padding row", children=[
            html.Div(className="six columns card", children=[
...
def update_pnl(exchange, leverage, start_date, end_date):
    data = []
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    for name, group in dff.groupby('Trade type'):
        data.append(
...
def update_balance(exchange, leverage, start_date, end_date):
    dff = filter_df(df, exchange, leverage, start_date, end_date)
    return {
        'data': [go.Scatter(x=dff['Entry time'], y=dff['Exit balance'] + dff['Pnl (incl fees)'])],
        'layout': {
...
def update_dates(value):
    return [
        df.loc[df['Exchange'] == value]['Entry time'].min(),
        df.loc[df['Exchange'] == value]['Entry time'].max()
    ]
